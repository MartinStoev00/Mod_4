	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			if (ForeignCharactersChecker.basicHasForeignCharacters(password)) {//sanitisation for password field
				...
				response.sendRedirect("/caren/login/error.html?error=" + error);
				return;
			}
			...
			if (resultset.next()) { // has an account
				...
				if (verified != 1) {
					String resendVerificationLink = "/caren/rest/resendEmail/" + email;
					response.sendRedirect("/caren/login/nverification.html?resend="+resendVerificationLink);
					return;
				}
				...
				if (pass.equals(hashedPassword)) {
					...
					response.sendRedirect("/caren/login/failure.html");
				}
			} else {
				//No accounts with given credentials.
				response.sendRedirect("/caren/login/failure.html");
